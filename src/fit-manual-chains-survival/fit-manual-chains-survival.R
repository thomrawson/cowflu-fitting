...
orderly2::orderly_parameters(n_samples = 100, n_particles = 8, n_chains = 2, step_size_var = 0.03, dt = 1,
                             ...
                             include_NAs = TRUE, chain_path = "Manual_samples_collection_DO_NOT_DELETE")
...
if(include_NAs){
  ...
  weeks_1_to_13 <- data.frame(
    ...
    outbreak_detected = I(lapply(1:13, function(x) rep(NA_real_, 48)))  # 48 NAs in each row
  )
  ...
  weeks_to_50 <- data.frame(
    ...
    week = (last_week + 1):50,
    outbreak_detected = I(lapply((last_week + 1):50, function(x) rep(NA_real_, 48)))
  )
...
resources2 <- hipercow_resources(cores = 32)

##Launch the first 30 sample gatherings
for(i in 1:n_chains){
  task_create_expr(monty::monty_sample_manual_run(i, chain_path),
...
repeat {
  # Check the condition
  if(monty::monty_sample_manual_info(chain_path)) {
    # If true, break the repeating loop
    break
  }
  # If false, wait 5 mins and try again
  Sys.sleep(300)
}



full_samples <- monty::monty_sample_manual_collect(chain_path, restartable = TRUE)
saveRDS(full_samples, file = "Full_samples_restartable.rds")
full_samples <- monty::monty_sample_manual_collect(chain_path)
saveRDS(full_samples, file = "Full_samples.rds")

monty::monty_sample_manual_cleanup(chain_path)
...
for(i in 1:length(plot_indices)){
  ...
  I_data <- full_samples$observations$history[49:96,,,]
...
##################################################################################################
## And now with the step function we fit to:
...
for(i in 1:length(plot_indices)){
  ...
  ## Extract the  data across all chains and iterations
  I_data <- full_samples$observations$history[1:48,,,]
  n_timepoints <- dim(I_data)[2]
  ## Reshape the array by collapsing dimensions 3 and 4
  I_data <- array(I_data, dim = c(48, n_timepoints, n_samples * n_chains))
  I_data <- I_data[regions_to_plot, , ]
  ...
  for (place in 1:dim(I_data)[1]) {      # Loop over the regions
    for (chain in 1:dim(I_data)[3]) {    # Loop over the iterations
      
      ## Get the time series for this place and chain
      infections <- I_data[place, , chain]
      ...
      if (!is.na(first_infection)) {
        ...
        I_data[place, first_infection:dim(I_data)[2], chain] <- 1
        
        ## Prior to the first infection, set the values to 0
        I_data[place, 1:(first_infection-1), chain] <- 0
      } else {
        ## If there are no infections at all, set everything to 0
        I_data[place, , chain] <- 0
      }
    }
  }
...
param_string <- sprintf("duration ran: %s mins\n
  ...
  restarts:  %s \n
  restart_every: %s \n
  include_NAs: %s ",   (duration$toc - duration$tic)/60,
                        n_samples,
